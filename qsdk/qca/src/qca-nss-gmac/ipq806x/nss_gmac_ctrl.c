...
int of_get_phy_mode_b(struct device_node *np)
{
	const char *pm;
	int err, i;
...
EXPORT_SYMBOL_GPL(of_get_phy_mode_b);
...
const void *of_get_mac_address_b(struct device_node *np)
{
	const void *addr;
...
EXPORT_SYMBOL(of_get_mac_address_b);
...
static int32_t nss_gmac_probe(struct platform_device *pdev)
{
	...
	if (test_bit(__NSS_GMAC_LINKPOLL, &gmacdev->flags)) {
		...
		nss_gmac_update_features((void *)&(gmacdev->phydev->supported),
					(void *)&(gmacdev->phydev->advertising));
